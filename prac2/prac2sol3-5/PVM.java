      ...
      do {
        ...
        switch (cpu.ir) {         // execute
          ...
          case PVM.mul:           // integer multiplication
            ...
            // checking if the value is too large (my change)
            sos = pop();
            if (tos != 0 && Math.abs(sos) > maxInt / Math.abs(tos)) {
              ps = badVal;
              break;
            }
            // if value is not too long, continue with mult
            else {
              push(sos * tos);
              break;
            }
          ...
          case PVM.div:           // integer division (quotient)
            tos = pop();
            if(tos == 0) { ps = divZero;    //division by zero not allowed (my change)
              break;
            }
            push(pop() / tos);
            break;
